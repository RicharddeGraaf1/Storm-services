...
def transform(int_path):
    ...
    # id-schema (mirror): integrated draagt uId = de wId letterlijk. STOP heeft een wId
    # dan-en-slechts-dan als het element een eId heeft (beide uit agAKN). Dus: heeft eId ->
    # wId = uId (byte-exact) vóór de bestaande (wId-lezende) logica. Elementen zonder eId
    # (Alinea, Rij/row, Cel/entry, Conditie) blijven uId-only — geen wId.
    for el in root.iter():
        if el.get('eId') is not None and el.get('uId') is not None:
            el.set('wId', el.get('uId'))
